					...
					<view class="detail-comment-item" v-for="(item,index) in songComment" :key="index">
						...
						<view class="detail-comment-content">
							<view class="detail-comment-title">
								...
								<view class="detail-comment-like">{{ item.likedCount | formatCount }} <text class="iconfont icon-dianzan_tuijian"></text></view>
							</view>
			...
			getMusic(songId){
				...
				Promise.all([ songDetail(songId) , songSimi(songId) , songComment(songId) , songLyric(songId) , songUrl(songId)]).then(res=>{
					...
					if(res[2][1].data.code == '200'){
						...
						this.comment = res[2][1].data.comments;
					}
					...
					if(res[4][1].data.code == '200'){
						...
						this.bgAudioManager.onPlay(()=>{
							...
							this.listenLyricIndex();//播放暂停时，清除定时器
						});
						this.bgAudioManager.onPause(()=>{
							this.iconPlay = 'icon-bofang';
							this.isPlayRotate = false;
							this.cancelLyricIndex();//播放暂停时，清除定时器
						});
...
<style scoped>
	...
	.detail-play view{width: 230rpx;height: 360rpx;background: url(~@/static/needle.png);position: absolute;top: -200rpx; left: 100rpx;right: 0;margin: auto;background-size: cover;}
	.detail-lyric{font-size: 32rpx;line-height: 82rpx;height: 246rpx;text-align: center;overflow: hidden;color:#c6c2bf;}
	.detail-lyric-wrap{transition: .5s;}
	.detail-lyric-item{height: 82rpx;text-overflow: clip;}
	.detail-lyric-item.active{color:white ;text-overflow: clip;}
